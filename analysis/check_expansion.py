...
def density_contour(S, T, grid_size, m, rs, snap, ngrid=128, delta_rho=False, figname='density_contour'):
    S0 = np.zeros_like(S)
    T0 = np.zeros_like(T)
    S0[0,0,0] = S[0,0,0]
    T0[0,0,0] = T[0,0,0]
    ...
    if delta_rho == False :
        ...
        im = ax.contourf(x0, y0, np.log10(np.abs(rho.value.reshape(ngrid, ngrid))), 20, cmap='inferno', 
                    origin='lower', extent=[-250, 250, -250, 250], vmin=-2,
                    vmax=5.5)
        plt.colorbar(im)
    elif delta_rho == True :
    ...
    plt.savefig(figname+'_{:03d}.png'.format(snap), bbox_inches='tight', dpi=300)
    plt.close()
